...
void SASBody::decode(const string &xml) {
...
const string SASBody::encode() {
  ...
  xmlTextWriterStartElementNS(writer, BAD_CAST _prefix, BAD_CAST "Body", nullptr);
  
  xmlTextWriterWriteElementNS(writer, BAD_CAST _prefix, BAD_CAST "Type", nullptr, BAD_CAST to_string(_type).c_str());
  ...
  if (!_mimeData.empty()) {
    xmlTextWriterWriteElementNS(writer, BAD_CAST _prefix, BAD_CAST "Data", nullptr, BAD_CAST _mimeData.c_str());
  }

  int32_t iTruncated = _isTruncated ? 1 : 0;
  xmlTextWriterWriteElementNS(writer, BAD_CAST _prefix, BAD_CAST "Truncated", nullptr, BAD_CAST to_string(iTruncated).c_str());
